  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" prop="createdAt" width="180">
          ...
          <el-table-column sortable align="left" label="项目ID" prop="projectId" width="120" />
          <el-table-column sortable align="left" label="构建号" prop="buildNumber" width="120" />
          <el-table-column sortable align="left" label="分支名" prop="branchName" width="120" />
          <el-table-column sortable align="left" label="提交ID" prop="commitId" width="120" />
         ...
         <el-table-column sortable align="left" label="结束时间" prop="endTime" width="180">
          ...
          <el-table-column sortable align="left" label="耗时" prop="duration" width="120" />
          <el-table-column sortable align="left" label="错误" prop="error" width="120" />
          <el-table-column align="left" label="构建类型" prop="buildType" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateCicdFunc(scope.row)">编辑</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="耗时:"  prop="duration" >
              <el-input v-model.number="formData.duration" :clearable="false" placeholder="请输入耗时" />
            </el-form-item>
...
const getTableData = async() => {
  const table = await getCicdList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCicdByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCicdFunc = async(row) => {
    const res = await findCicd({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCicdFunc = async (row) => {
    const res = await deleteCicd({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
